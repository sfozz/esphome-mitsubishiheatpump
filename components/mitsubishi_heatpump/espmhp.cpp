...
void MitsubishiHeatPump::updateDevice() {
    ...
    if (this->remote_temperature > 0) {
        ESP_LOGI(TAG, "Current: Using remote temperature %.2f", this->remote_temperature);
        this->current_temperature = this->remote_temperature;
    } else {
        ESP_LOGI(TAG, "Current: Using device temperature %.2f", deviceStatus.currentTemperature);
